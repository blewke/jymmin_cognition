...
priors_shift = c(
  prior(gamma(2,1), class = Intercept, dpar = 'shift'),
  prior(normal(0,0.25), class = sd,group = 'SubjectCode', dpar = 'shift')
)



priorModelA_shift = c(priors_duration, priors_shift)

ModelA =  brm (
  data = study_data_timed,
  formula =
    bf (DurationInSeconds|vint(ClocksInSet) ~ 1 + nTrialScale + (1 + nTrialScale|s|SubjectCode) + (1 + nTrialScale|l|Level),
        log(shift)  ~ 1 + (1|s|SubjectCode),
        family = sum_shifted_lognormal),
  prior = priorModelA_shift,
  #backend = 'cmdstanr',
  sample_prior = 'yes',
  ...
  iter = 1000,
  seed = 7,
  file = '../results/ModelA_variableshift',
